...
from testdata.movies_data.get_by_film_id import (
    expected_film_data,
    expect_not_found_film_data,
)
...
class TestMovies:
    ...
    @pytest.mark.parametrize("method, query, expected_status", [*film_list_params])
    async def test_get_search_film(
        self, make_get_request, redis_client, method, query, expected_status
    ):
